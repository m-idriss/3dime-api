...
Successful converter responses may include a `warnings` array. These warnings are non-blocking,
machine-readable review hints such as `timezone_assumed:event[0]:Europe/Paris`. Calendar output
has already been parsed, validated and regenerated by the API before it is returned.
